...
export const metadata: Metadata = getMetadata({
  title: '경기 북부 창틀코킹 시공 서비스 지역 안내',
  description: '고양시, 파주시, 양주시 등 경기 북부 지역의 창틀코킹, 창틀누수, 외벽누수 정밀 시공 페이지 리스트입니다.',
  path: '/sitemap',
  noIndex: false, // noindex 예외 전수 제거
});
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <header className="mb-16 text-center max-w-2xl mx-auto space-y-4">
            ...
            <p className="text-[14.5px] text-slate-500 leading-relaxed">
              고양시, 파주시, 양주시 전 지역의 서비스 키워드를 안내해 드립니다.<br className="hidden sm:block" />
              아래의 원하시는 시공 부위와 지역을 클릭하시면 전용 진단 페이지로 즉시 연결됩니다.
            </p>
          </header>
          ...
          <div className="space-y-12">
            {REGIONS_DB.map((region, idx) => {
              // 해당 시의 49개 지역명 리스트와 6대 서비스 조합 빌드 (중복 제거)
              const combinedKeywords: { label: string; area: string }[] = [];
              
              region.areas.forEach(area => {
                SERVICES.forEach(service => {
                  combinedKeywords.push({ label: `${area}-${service}`, area: area });
                });
              });
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-3.5 mb-8 pb-4 border-b border-slate-100">
                    ...
                    <h2 className="text-2.5xl font-black text-slate-800 tracking-tight">
                      {region.city} 시공 안내 ({region.areas.length}개 세부 지역)
                    </h2>
                  </div>
