...
export const getDynamicHomeData = (region: string, service: string, hash: number) => {
  ...
  const configs: Record<string, DynamicHomeConfig> = {
    '창틀코킹': {
      ...
      faqs: [
        { 
          question: `${region} ${service}은 언제 점검이 필요할까요?`, 
        ...
        { 
          ...
          answer: `실리콘이 노화되면 인장 강도가 저하되어 샷시와 콘크리트 사이에 틈이 생깁니다. 이 틈으로 유입된 물이 내벽으로 침투하면 도배지가 들뜨고 곰팡이가 피어 주거 환경이 손상되며, 방치할 경우 콘크리트 내부 철근 부식으로 이어질 수 있습니다.` 
        },
        { 
    ...
    '창틀누수': {
      ...
      faqs: [
        ...
        { 
          question: `창틀 실리콘이 노후되면 어떤 문제가 생기나요?`, 
          answer: `실내외 온도차와 습도 노출로 실리콘이 노화되면 샷시 주변에 균열 틈새가 생깁니다. 틈새로 유입된 물이 내벽으로 침투하면 도배지가 들뜨고 곰팡이가 피어 주거 환경이 손상되며, 방치할 경우 건물 구조 손상으로 이어집니다.` 
        },
        { 
          question: `${region} 지역 외벽 크랙도 함께 확인 가능한가요?`, 
          answer: `네, 가능합니다. 틈새케어는 샷시 주변 보강에만 머무르지 않고, 창틀 상부의 수직 균열 및 외벽 크랙을 함께 전수 검사합니다. 균열이 확인되면 외부 전용 크랙 신축 보강재를 주입하여 콘크리트 내부로 유입되는 누수 요소를 일괄 폐쇄해 드립니다.` 
